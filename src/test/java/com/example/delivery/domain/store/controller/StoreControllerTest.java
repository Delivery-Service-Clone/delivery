...
class StoreControllerTest {
  ...
  void testGetStores() throws Exception {
    // Given
    StoreDto storeDto1 = new StoreDto(1L, "Burger King", "456 Elm St", "123-456-7890", StoreStatus.STORE_OPEN, "Best burgers", Category.BURGER);
    StoreDto storeDto2 = new StoreDto(2L, "Korean BBQ", "123 Main St", "111-111-1111", StoreStatus.STORE_OPEN, "Delicious Korean BBQ", Category.KOREAN);
    List<StoreDto> stores = Arrays.asList(storeDto1, storeDto2);

    when(storeService.getAllStores()).thenReturn(stores);

    // When & Then
    mockMvc.perform(get("/api/v1/stores/list")
  ...
  void testChangeStoreStatus() throws Exception {
    ...
    // When & Then
    mockMvc.perform(put("/api/v1/stores/{storeId}/status", storeId)
            .contentType(MediaType.APPLICATION_JSON)
